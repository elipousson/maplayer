...
- Replace the deprecated `label.size` argument of `geom_label()`/`geom_sf_label()` with `linewidth` in `layer_numbers()`. Also fix the internal `modify_fn_params()` helper (used by `layer_location_data()` to build every geom call) to drop any target-function formal whose default is the `lifecycle::deprecated()` sentinel instead of forwarding it unevaluated, which had been silently re-triggering the `label.size` deprecation warning on every `"label"`/`"sf_label"` geom call, including via `layer_numbers()`, `layer_markers()`, `layer_labelled()`, and `make_image_map()`, even when `label.size` was never referenced.
